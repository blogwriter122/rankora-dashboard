...
export default function ReachCampaigns() {
  ...
  async function launch() {
    ...
    try {
      ...
      setKeywords(''); setTimeout(() => setMsg(''), 5000);
    } catch(e) { alert(e.message); }
  }
  const byPlatform = published.reduce((a,p) => { a[p.platform]=(a[p.platform]||0)+1; return a; }, {});
  return (
    <div className="fade-in">
      <div className="page-head"><div><div className="flex" style={{gap:10}}><div className="page-title">Campaigns</div><span className="engine-tag reach">Reach</span></div><div className="page-sub">Publish to high-DA platforms → funnel traffic to your money site</div></div>{msg && <span className="badge go">{msg}</span>}</div>
      <div className="grid grid-2">
        <div className="panel">
          <div className="panel-title">⇶ Launch Campaign</div>
          <div className="field"><label>Keywords (one per line)</label><textarea rows={7} value={keywords} onChange={e=>setKeywords(e.target.value)} placeholder={'best ai writing tools\nnotion vs obsidian\nhow to start a blog'} /></div>
          <div className="row"><div className="field"><label>Niche</label><input value={niche} onChange={e=>setNiche(e.target.value)} placeholder="ai tools" /></div><div className="field"><label>Money Site URL</label><input value={ownSite} onChange={e=>setOwnSite(e.target.value)} placeholder="https://mysite.com" /></div></div>
          <label>Platforms</label>
          <div className="flex" style={{flexWrap:'wrap',marginBottom:16}}>
            {PLATFORMS.map(p=><button key={p} className={`btn sm ${selected.includes(p)?'reach':'ghost'}`} onClick={()=>toggle(p)} style={{margin:3}}>{p}</button>)}
          </div>
          <button className="btn reach" onClick={launch}>Launch to {selected.length} Platforms</button>
        </div>
        <div className="panel">
          ...
          {Object.keys(byPlatform).length === 0 ? <div className="empty">Nothing published yet.</div> : (
            <table><thead><tr><th>Platform</th><th>Articles</th></tr></thead>
            <tbody>{Object.entries(byPlatform).sort((a,b)=>b[1]-a[1]).map(([p,c])=>(
      ...
      <div className="panel" style={{marginTop:14}}>
        ...
        {published.length === 0 ? <div className="empty">No publications yet.</div> : (
          <table><thead><tr><th>Keyword</th><th>Platform</th><th>URL</th><th>When</th></tr></thead>
          <tbody>{published.slice(0,15).map(p=>(
            <tr key={p.id}>
